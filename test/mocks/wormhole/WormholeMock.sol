...
import { BytesLib } from "src/dependencies/wormhole/util/BytesLib.sol";
...
contract WormholeMock is IWormhole {
    ...
    function governanceActionIsConsumed(bytes32 /*hash*/ ) external pure override returns (bool) {
        return false;
    }

    function isInitialized(address /*impl*/ ) external pure override returns (bool) {
        return true;
    }
    ...
    function verifyVM(VM memory /*vm*/ ) external pure override returns (bool, /*valid*/ string memory /*reason*/ ) {
    ...
    function submitContractUpgrade(bytes memory /*_vm*/ ) external pure override {
        revert("unsupported submitContractUpgrade in wormhole mock");
    }

    function submitSetMessageFee(bytes memory /*_vm*/ ) external pure override {
    ...
    function submitNewGuardianSet(bytes memory /*_vm*/ ) external pure override {
        revert("unsupported submitNewGuardianSet in wormhole mock");
    }

    function submitTransferFees(bytes memory /*_vm*/ ) external pure override {
        revert("unsupported submitTransferFees in wormhole mock");
    }

    function submitRecoverChainId(bytes memory /*_vm*/ ) external pure override {
