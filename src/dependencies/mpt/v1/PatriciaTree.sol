...
import { D } from "src/dependencies/mpt/v1/PatriciaTreeData.sol";
import { Utils } from "src/dependencies/mpt/v1/PatriciaTreeUtils.sol";
...
library PatriciaTree {
    ...
    function getNode(Tree storage tree, bytes32 hash)
        internal
        view
        returns (uint256, bytes32, bytes32, uint256, bytes32, bytes32)
    {
    ...
    function getRootEdge(Tree storage tree) internal view returns (uint256, bytes32, bytes32) {
    ...
    function getProof(Tree storage tree, bytes memory key)
        internal
        view
    ...
    function getProofWithHashedKey(Tree storage tree, bytes32 hashedKey)
        internal
        view
        returns (uint256 branchMask, bytes32[] memory _siblings)
    {
    ...
    function getNonInclusionProof(Tree storage tree, bytes memory key)
        internal
        view
        returns (
    ...
    function getNonInclusionProofWithHashedKey(Tree storage tree, bytes32 hashedKey)
        internal
        view
        returns (
            bytes32 potentialSiblingLabel,
    ...
    function verifyProof(
        ...
        for (uint256 i = 0; branchMask != 0; i++) {
            uint256 bitSet = Utils.lowestBitSet(branchMask);
    ...
    function _insertAtNode(Tree storage tree, bytes32 nodeHash, D.Label memory key, bytes32 value)
        private
        returns (bytes32)
    {
    ...
    function _insertAtEdge(Tree storage tree, D.Edge memory e, D.Label memory key, bytes32 value)
        private
        returns (D.Edge memory)
    {
