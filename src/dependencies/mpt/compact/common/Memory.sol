...
library Memory {
     ...
     */
    function equals(uint256 addr_, uint256 addr2_, uint256 len_) internal pure returns (bool equal_) {
        assembly {
     ...
     * @return equal_ 'true' if the bytes are the same, otherwise 'false'.
     */
    function equals(uint256 addr_, uint256 len_, bytes memory bts_) internal pure returns (bool equal_) {
     ...
     */
    function dataPtr(bytes memory bts_) internal pure returns (uint256 addr_) {
        assembly {
     ...
     */
    function toBytes(uint256 addr_, uint256 len_) internal pure returns (bytes memory bts_) {
        bts_ = new bytes(len_);
        uint256 btsptr_;
        assembly {
     ...
     * @param len_ The number of bytes to copy.
     */
    function copy(uint256 src_, uint256 dest_, uint256 len_) internal pure {
        ...
        uint256 mask_ = 256 ** (WORD_SIZE - len_) - 1;
        assembly {
     ...
     */
    function fromBytes(bytes memory bts_) internal pure returns (uint256 addr_, uint256 len_) {
        len_ = bts_.length;
        assembly {
            addr_ := add(bts_, BYTES_HEADER_SIZE)
        }
